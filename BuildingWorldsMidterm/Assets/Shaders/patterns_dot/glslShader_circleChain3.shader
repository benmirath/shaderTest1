Shader "Unlit/glslShader_circleChain3"
{
	Properties {
...
//				    color += d;
				    float time2 = _Time.y * .15;

//				    st *= 20.;	//repeat 10 times
					if( fract(time2)>0.5 ){
					    st = movingTiles (st, 20., .15);
					} else {
						st = movingTiles (st, 20., 0.);
					}
				    vec2 st_i = floor(st);
//				    st_i -= .5;
...
//					    st_f += .5;
				    ...
				    st_f -= .5;
//				    st_f = rotationMatrix(d*3.14)*st_f;
					st_f = rotationMatrix(PI *0.5)*st_f;
				    st_f += .5;
					float timer = abs ( sin (_Time.y * .45 ) );
//					timer = abs ( sin (timer) );
					vec2 position = vec2 (0.5);
				    
				    vec2 offset1 = vec2 (1., 1.) * timer;
				    vec2 offset2 = vec2 (-1., 1.) * timer;
				    vec2 offset3 = vec2 (1., -1.) * timer;
				    vec2 offset4 = vec2 (-1., -1.) * timer;
...
//						offset4 = vec2 (0.);					
					...
					pct += circle (st_f + offset1, 0.25);
					pct += circle (st_f + offset2, 0.25);
					pct += circle (st_f + offset3, 0.25);
					pct += circle (st_f + offset4, 0.25);
					pct = clamp (pct, 0., 1.);	//use to limit color from going full white
					
				    col = mix (_Color1, _Color2, pct + abs (sin (_Time.y * .25)));
					col = _Color1 * pct;
